...
Now let's say you want to have those same two input elements, but this time you want elements for each of them.
This presents a problem, you now need to be able to reference the same ID twice. One for the 's `for` attribute, and the other for the `id` on the input.
