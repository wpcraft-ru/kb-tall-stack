...
Let's say you wanted to listen for the `enter` key to be pressed inside an element. Alpine makes this easy by adding the `.enter` like so:
```
<input @keyup.enter="...">
```
...
```
<button @click="$event.target.remove()">Remove Mebutton>
```
...
div>
```
As you can see, when the button is clicked, Alpine will dispatch a browser event called "foo", and our `@foo` listener on the will pick it up and react to it.
