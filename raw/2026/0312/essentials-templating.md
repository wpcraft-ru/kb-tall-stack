...
div>
```
Now, Alpine will set the text content of the `#` with the value of `title` ("Start Here"). When `title` changes, so will the contents of `#`.
...
<button @click="open = ! open">Expandbutton>
<div x-show="open">
Content...
div>
div>
```
Content...
Now the entire containing the contents will be shown and hidden based on the value of `open`.
...
div>
```
Notice that `x-if` must be declared on a tag. This is so that Alpine can leverage the existing browser behavior of the element and use it as the source of the target to be added and removed from the page.
When `open` is true, Alpine will append the to the tag, and remove it when `open` is false.
...
```
<div x-data="{ open: false }">
<button @click="open = ! open">Expandsbutton>
...
div>
```
Similar to `x-if`, `x-for` must be applied to a tag. Internally, Alpine will append the contents of tag for every iteration in the loop.
...
div>
```
Now, Alpine will set the text content of the with the element `# Start Here`. When `title` changes, so will the contents of `#`.
