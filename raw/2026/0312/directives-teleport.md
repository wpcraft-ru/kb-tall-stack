...
By attaching `x-teleport` to a element, you are telling Alpine to "append" that element to the provided selector.
...
To make this experience more seamless, you can "forward" events by simply registering event listeners on the element itself like so:
```
<div x-data="{ open: false }">
<button @click="open = ! open">Toggle Modalbutton>
...
div>
```
Notice how we are now able to listen for events dispatched from within the teleported element from outside the element itself?
...
Modal contents...
<div x-data="{ open: false }">
<button @click="open = ! open">Toggle Nested Modalbutton>
<template x-teleport="body">
<div x-show="open">
