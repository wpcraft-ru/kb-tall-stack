...
use Livewire\Component;new class extends Component { public $count = 1; public function increment() { $this->count++; } public function render() { return <<<'HTML' <div> Count: {{ $count }} <button wire:click="increment">+button> div> HTML; }};
...
use Livewire\Component;new class extends Component { public $todos; public function mount() { $this->todos = collect([ 'first', 'second', 'third', ]); }};
...
use Livewire\Component;new class extends Component { public $todos; public function mount() { $this->todos = collect([ 'first', 'second', str('third'), ]); }};
...
• Lifecycle Hooks — Use hydrate() and dehydrate() hooks
• Properties — How properties are preserved between requests
• Morphing — How Livewire updates the DOM
• Synthesizers — Customize property serialization
