...
<form wire:submit="save"> <input type="text" wire:model="title"> <div>@error('title') {{ $message }} @enderrordiv> <textarea wire:model="content">textarea> <div>@error('content') {{ $message }} @enderrordiv> <button type="submit">Savebutton>form>
<input type="text" wire:model="title">
<div>@error('title') {{ $message }} @enderrordiv>
<textarea wire:model="content">textarea>
<div>@error('content') {{ $message }} @enderrordiv>
...
namespace App\Livewire\Forms;use Livewire\Attributes\Validate;use Livewire\Form;class PostForm extends Form{ #[Validate('required|min:3')] public $title = ''; #[Validate('required|min:3')] public $content = '';}
use Livewire\Attributes\Validate;
The PostForm above can now be defined as a property on the CreatePost component:
namespace App\Livewire;use App\Livewire\Forms\PostForm;use Livewire\Component;use App\Models\Post;class CreatePost extends Component{ public PostForm $form; public function save() { Post::create( $this->form->all() ); return redirect()->to('/posts'); } // ...}
...
<form wire:submit="save"> <input type="text" wire:model="form.title"> <div>@error('form.title') {{ $message }} @enderrordiv> <textarea wire:model="form.content">textarea> <div>@error('form.content') {{ $message }} @enderrordiv> <button type="submit">Savebutton>form>
<input type="text" wire:model="form.title">
<div>@error('form.title') {{ $message }} @enderrordiv>
<textarea wire:model="form.content">textarea>
<div>@error('form.content') {{ $message }} @enderrordiv>
<button type="submit">Savebutton>
...
return redirect()->to('/posts');
Therules() method doesn't validate on data updates
...
namespace Tests\Feature\Livewire;use App\Livewire\CreatePost;use Livewire\Livewire;use Tests\TestCase;class CreatePostTest extends TestCase{ public function test_cant_create_post_without_title() { Livewire::test(CreatePost::class) ->set('content', 'Sample content...') ->call('save') ->assertHasErrors('title'); }}
...
<form wire:submit="save"> <input type="email" wire:model="email"> <div wire:show="$errors.has('email')"> <span wire:text="$errors.first('email')">span> div> <button type="submit">Savebutton>form>
...
• Forms — Validate form inputs with real-time feedback
• Properties — Validate property values before persisting
• Validate Attribute — Use #[Validate] for property validation
• Actions — Validate data in action methods
