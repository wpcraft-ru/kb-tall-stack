...
// resources/views/components/post/⚡create.blade.phpuse Livewire\Component;use App\Models\Post;new class extends Component { public $title = ''; public $content = ''; public function save() { Post::create( $this->only(['title', 'content']) ); session()->flash('status', 'Post successfully updated.'); return $this->redirect('/posts'); }};?><form wire:submit="save"> <input type="text" wire:model="title"> <input type="text" wire:model="content"> <button type="submit">Savebutton>form>
...
<form wire:submit="save"> <input type="text" wire:model="title"> <div> @error('title') <span class="error">{{ $message }}span> @enderror div> <input type="text" wire:model="content"> <div> @error('content') <span class="error">{{ $message }}span> @enderror div> <button type="submit">Savebutton>form>
<input type="text" wire:model="title">
...
namespace App\Livewire\Forms;use Livewire\Attributes\Validate;use Livewire\Form;class PostForm extends Form{ #[Validate('required|min:5')] public $title = ''; #[Validate('required|min:5')] public $content = '';}
use Livewire\Attributes\Validate;
// resources/views/components/post/⚡create.blade.phpuse App\Livewire\Forms\PostForm;use Livewire\Component;use App\Models\Post;new class extends Component { public PostForm $form; public function save() { $this->validate(); Post::create( $this->form->only(['title', 'content']) ); return $this->redirect('/posts'); }};
// resources/views/components/post/⚡create.blade.php
use App\Livewire\Forms\PostForm;
$this->form->only(['title', 'content'])
return $this->redirect('/posts');
<form wire:submit="save"> <input type="text" wire:model="form.title"> <div> @error('form.title') <span class="error">{{ $message }}span> @enderror div> <input type="text" wire:model="form.content"> <div> @error('form.content') <span class="error">{{ $message }}span> @enderror div> <button type="submit">Savebutton>form>
...
namespace App\Livewire\Forms;use Livewire\Attributes\Validate;use App\Models\Post;use Livewire\Form;class PostForm extends Form{ #[Validate('required|min:5')] public $title = ''; #[Validate('required|min:5')] public $content = ''; public function store() { $this->validate(); Post::create($this->only(['title', 'content'])); }}
use Livewire\Attributes\Validate;
Post::create($this->only(['title', 'content']));
Now you can call $this->form->store() from the component:
// resources/views/components/post/⚡create.blade.phpuse App\Livewire\Forms\PostForm;use Livewire\Component;new class extends Component { public PostForm $form; public function save() { $this->form->store(); return $this->redirect('/posts'); } // ...};
// resources/views/components/post/⚡create.blade.php
use App\Livewire\Forms\PostForm;
return $this->redirect('/posts');
...
// resources/views/components/post/⚡edit.blade.phpuse App\Livewire\Forms\PostForm;use Livewire\Component;use App\Models\Post;new class extends Component { public PostForm $form; public function mount(Post $post) { $this->form->setPost($post); } public function save() { $this->form->update(); return $this->redirect('/posts'); }};
// resources/views/components/post/⚡edit.blade.php
use App\Livewire\Forms\PostForm;
public function mount(Post $post)
return $this->redirect('/posts');
namespace App\Livewire\Forms;use Livewire\Attributes\Validate;use Livewire\Form;use App\Models\Post;class PostForm extends Form{ public ?Post $post; #[Validate('required|min:5')] public $title = ''; #[Validate('required|min:5')] public $content = ''; public function setPost(Post $post) { $this->post = $post; $this->title = $post->title; $this->content = $post->content; } public function store() { $this->validate(); Post::create($this->only(['title', 'content'])); } public function update() { $this->validate(); $this->post->update( $this->only(['title', 'content']) ); }}
use Livewire\Attributes\Validate;
...
namespace App\Livewire\Forms;use Livewire\Attributes\Validate;use App\Models\Post;use Livewire\Form;class PostForm extends Form{ #[Validate('required|min:5')] public $title = ''; #[Validate('required|min:5')] public $content = ''; // ... public function store() { $this->validate(); Post::create($this->only(['title', 'content'])); $this->reset(); }}
use Livewire\Attributes\Validate;
Post::create($this->only(['title', 'content']));
...
namespace App\Livewire\Forms;use Livewire\Attributes\Validate;use App\Models\Post;use Livewire\Form;class PostForm extends Form{ #[Validate('required|min:5')] public $title = ''; #[Validate('required|min:5')] public $content = ''; // ... public function store() { $this->validate(); Post::create( $this->pull() ); }}
use Livewire\Attributes\Validate;
...
namespace App\Livewire\Forms;use Livewire\Attributes\Validate;use Illuminate\Validation\Rule;use App\Models\Post;use Livewire\Form;class PostForm extends Form{ public ?Post $post; #[Validate] public $title = ''; public $content = ''; protected function rules() { return [ 'title' => [ 'required', Rule::unique('posts')->ignore($this->post), ], 'content' => 'required|min:5', ]; } // ... public function update() { $this->validate(); $this->post->update($this->only(['title', 'content'])); $this->reset(); }}
use Livewire\Attributes\Validate;
use Illuminate\Validation\Rule;
Rule::unique('posts')->ignore($this->post),
'content' => 'required|min:5',
...
<button type="submit"> <span class="in-data-loading:hidden">Savespan> <span class="not-in-data-loading:hidden"> <svg>...svg> span>button>
...
<input type="text" wire:model.live.blur="title"><div> @error('title') <span class="error">{{ $message }}span> @enderrordiv>
<input type="text" wire:model.live.blur="title">
@error('title') <span class="error">{{ $message }}span> @enderror
...
@error('content') <span class="error">{{ $message }}span> @enderror
<button type="submit">Savebutton>
Here's what the template will look like after extracting a re-usable Blade component called :
<form wire:submit="save"> <x-input-text name="title" wire:model="title" /> <x-input-text name="content" wire:model="content" /> <button type="submit">Savebutton>form>
...
@props(['name'])<input type="text" name="{{ $name }}" {{ $attributes }}><div> @error($name) <span class="error">{{ $message }}span> @enderrordiv>
...
This ensures wire:model="title" and any other extra attributes such as disabled, class="...", or required still get forwarded to the actual element.
...
For example, let's imagine you wanted to create an component that was a simple "counter" input written in Alpine.
...
Now, let's imagine we want to extract this component into a Blade component called that we would use within a component like so:
...
<div x-data="{ count: 0 }" x-modelable="count" {{ $attributes}}> <button x-on:click="count--">-button> <span x-text="count">span> <button x-on:click="count++">+button>div>
...
Because of {{ $attributes }}, when the HTML is rendered in the browser, wire:model="quantity" will be rendered alongside x-modelable="count" on the root
...
• Validation — Validate form inputs with real-time feedback
• wire:model — Bind form inputs to component properties
• File Uploads — Handle file uploads in forms
• Actions — Process form submissions with actions
