...
// resources/views/components/⚡show-user.blade.phpuse Illuminate\Support\Facades\Auth;use Livewire\Attributes\Computed;use Livewire\Component;use App\Models\User;new class extends Component { public $userId; #[Computed] public function user() { return User::find($this->userId); } public function follow() { Auth::user()->follow($this->user); }};
// resources/views/components/⚡show-user.blade.php
...
Computed properties are not supported onLivewire\Form objects.
...
• You access a computed property that depends on a certain property or database state
• The underlying property or database state changes
• The memoized value for the property becomes stale and needs to be re-computed
...
// resources/views/components/⚡show-posts.blade.phpuse Illuminate\Support\Facades\Auth;use Livewire\Attributes\Computed;use Livewire\Component;new class extends Component { public function createPost() { if ($this->posts->count() > 10) { throw new \Exception('Maximum post count exceeded'); } Auth::user()->posts()->create(...); unset($this->posts); } #[Computed] public function posts() { return Auth::user()->posts; } // ...};
...
// resources/views/components/⚡show-user.blade.phpuse Illuminate\Support\Facades\Cache;use Livewire\Attributes\Computed;use Livewire\Component;use App\Models\User;new class extends Component { public $userId; #[Computed] public function user() { $key = 'user'.$this->getId(); $seconds = 3600; // 1 hour... return Cache::remember($key, $seconds, function () { return User::find($this->userId); }); } // ...};
// resources/views/components/⚡show-user.blade.php
...
#[Computed(persist: true, seconds: 7200)]
Callingunset() will clear both memo and cache
...
<div> @foreach ($posts as $post) <div wire:key="{{ $post->id }}"> div> @endforeachdiv>
...
<div> @if (Auth::user()->can_see_posts) @foreach ($posts as $post) <div wire:key="{{ $post->id }}"> div> @endforeach @endifdiv>
...
use Livewire\Attributes\Computed;
return view('livewire.show-posts');
<div> @if (Auth::user()->can_see_posts) @foreach ($this->posts as $post) <div wire:key="{{ $post->id }}"> div> @endforeach @endifdiv>
@if (Auth::user()->can_see_posts)
...
// resources/views/components/⚡show-posts.blade.phpuse Livewire\Attributes\Computed;use Livewire\Component;use App\Models\Post;new class extends Component { #[Computed] public function posts() { return Post::all(); } public function render() { return <<<HTML <div> @foreach ($this->posts as $post) <div wire:key="{{ $post->id }}"> div> @endforeach div> HTML; }};
// resources/views/components/⚡show-posts.blade.php
use Livewire\Attributes\Computed;
@foreach ($this->posts as $post)
<div wire:key="{{ $post->id }}">
...
// resources/views/components/⚡show-posts.blade.phpuse Livewire\Attributes\Computed;use Livewire\Component;use App\Models\Post;new class extends Component { #[Computed] public function posts() { return Post::all(); }};
// resources/views/components/⚡show-posts.blade.php
use Livewire\Attributes\Computed;
<div> @foreach ($this->posts as $post) <div wire:key="{{ $post->id }}"> div> @endforeachdiv>
@foreach ($this->posts as $post)
<div wire:key="{{ $post->id }}">
Alternative: Session properties
If you need to persist simple values across page refreshes without cross-request caching, consider using the #[Session] attribute instead of computed properties.
...
• Properties — Understand basic property management
• Islands — Optimize performance with lazy computed values
• Computed Attribute — Use #[Computed] for memoization
• Components — Access computed properties in views
